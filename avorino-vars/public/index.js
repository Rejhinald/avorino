...
async function safeCall(label, fn, retries = 2) {
...
// ════════════════════════════════════════════════════════════════
async function getOrCreateStyle(name) {
...
async function setProps(style, name, props) {
...
async function freshStyle(name) {
...
async function clearAndSet(style, name, props) {
    try {
...
async function clearAndSetBatch(style, name, props) {
    try {
        await style.removeAllProperties();
        await wait(200);
    }
...
async function setSharedStyleProps(styles, v) {
...
document.getElementById('build-about')?.addEventListener('click', async () => {
    const btn = document.getElementById('build-about');
    btn.disabled = true;
    try {
