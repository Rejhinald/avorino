...
const HEAD_CODE = [
  '<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/Rejhinald/avorino@c7533aa/avorino-responsive.css">',
  '<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/Rejhinald/avorino@c7533aa/avorino-nav-footer.css">',
  CALENDLY_CSS,
].join('\n');
const FOOTER_CODE = [
...
async function buildCityADUPage() {
  ...
  const { body } = await createPageWithSlug(
    `${CITY.city} ADU`, CITY.slug,
    `ADU Construction in ${CITY.city} — Avorino`,
    `ADU construction in ${CITY.city}, Orange County. Fully permitted, designed, and built by Avorino.`,
  );
...
document.getElementById('build-page')?.addEventListener('click', async () => {
  ...
  btn.disabled = true;
  try { await buildCityADUPage(); } catch (err: any) {
