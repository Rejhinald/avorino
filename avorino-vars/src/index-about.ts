...
const HEAD_CODE = [
  '<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/Rejhinald/avorino@46289e0/avorino-about-head.css">',
  '<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/Rejhinald/avorino@46289e0/avorino-responsive.css">',
  CALENDLY_CSS,
].join('\n');
const FOOTER_CODE = [
  ...
  '<script src="https://cdn.jsdelivr.net/gh/Rejhinald/avorino@46289e0/avorino-about-footer.js"><\/script>',
  '<script src="https://cdn.jsdelivr.net/gh/Rejhinald/avorino@46289e0/avorino-about-mv3d.js"><\/script>',
  '<script src="https://cdn.jsdelivr.net/gh/Rejhinald/avorino@46289e0/avorino-about-values3d.js"><\/script>',
  CALENDLY_JS,
].join('\n');
...
async function buildAboutPage() {
  ...
  hero.setTag('section'); hero.setStyles([heroSection]); hero.setAttribute('id', 'about-hero');
  ...
  story.setTag('section'); story.setStyles([s.section, s.sectionWarm]); story.setAttribute('id', 'about-story');
  const storyLbl = story.append(webflow.elementPresets.DOM); storyLbl.setTag('div'); storyLbl.setStyles([s.label, mb64]); storyLbl.setAttribute('data-animate', 'fade-up');
  const storyLblTxt = storyLbl.append(webflow.elementPresets.DOM); storyLblTxt.setTag('div'); storyLblTxt.setTextContent('Our Story');
  const storyLblLine = storyLbl.append(webflow.elementPresets.DOM); storyLblLine.setTag('div'); storyLblLine.setStyles([labelLine]);
  const storyGrid = story.append(webflow.elementPresets.DOM); storyGrid.setTag('div'); storyGrid.setStyles([s.grid2]);
  const storyLeft = storyGrid.append(webflow.elementPresets.DOM); storyLeft.setTag('div'); storyLeft.setAttribute('data-animate', 'fade-up');
  const storyImgEl = storyLeft.append(webflow.elementPresets.DOM); storyImgEl.setTag('div'); storyImgEl.setStyles([storyImg]);
  const storyRight = storyGrid.append(webflow.elementPresets.DOM); storyRight.setTag('div'); storyRight.setStyles([storyBody]);
  const storyH = storyRight.append(webflow.elementPresets.DOM); storyH.setTag('h2'); storyH.setStyles([s.headingLG, maxWidth580, mb48]);
  ...
  mvCanvasWrap.setTag('div'); mvCanvasWrap.setAttribute('class', 'mv-canvas-wrap');

  // Phrase layer — big text that fills viewport first
  const mvPhraseLayer = mv.append(webflow.elementPresets.DOM);
  mvPhraseLayer.setTag('div'); mvPhraseLayer.setAttribute('class', 'mv-phrase-layer');
  ...
  missionPanel.setTag('div'); missionPanel.setAttribute('class', 'mv-panel');
  ...
  mBody.setTag('p'); mBody.setAttribute('class', 'mv-panel-body');
  ...
  visionPanel.setTag('div'); visionPanel.setAttribute('class', 'mv-panel');
  ...
  vBody.setTag('p'); vBody.setAttribute('class', 'mv-panel-body');
  ...
  const valsLbl = vals.append(webflow.elementPresets.DOM); valsLbl.setTag('div'); valsLbl.setStyles([s.label, mb64]); valsLbl.setAttribute('data-animate', 'fade-up');
  const valsLblTxt = valsLbl.append(webflow.elementPresets.DOM); valsLblTxt.setTag('div'); valsLblTxt.setTextContent('Our Values');
...
document.getElementById('build-page')?.addEventListener('click', async () => {
  ...
  btn.disabled = true;
  try { await buildAboutPage(); } catch (err: any) {
